...
class MetricRelayTest(unittest2.TestCase):
    ...
    def test_custom_counter_with_default_attributes(self):
        sock = self.patched[
            'shumway.socket.socket'].mock_instance
        ...
        metric = {'key': 'key',
                  'attributes': {'foo': 'bar', 'what': 'test', 'k': 'v'},
                  'value': 1,
                  'type': 'metric',
                  'tags': [],
                  'resources': {}}
    ...
    def test_custom_counter_with_default_resources(self):
        sock = self.patched[
            'shumway.socket.socket'].mock_instance

        resources = dict(foo='bar')
        mr = shumway.MetricRelay('key', default_resources=resources)
        mr.set_counter(
            'test', shumway.Counter('test', 'key',
                                    resources={'k': 'v'}))
        mr.incr('test')
    ...
    def test_custom_timer_with_default_metric_relay_attributes(self):
        sock = self.patched[
            'shumway.socket.socket'].mock_instance

        timer = shumway.Timer('test-timer', 'key')
        timer.update(10)
    ...
    def test_custom_timer_with_default_metric_relay_resources(self):
        sock = self.patched[
            'shumway.socket.socket'].mock_instance
